...
- (id) callWithArguments:(id)cdr context:(NSMutableDictionary *)context
{
    ...
    if ([split count] == 2) {
        ...
        id fileName = getPathInDocs(resourceName);
        if (![[NSFileManager defaultManager] fileExistsAtPath:fileName]) {
            fileName = getPathInDocs([NSString stringWithFormat:@"%@.nu", resourceName]);
            if (![[NSFileManager defaultManager] fileExistsAtPath:fileName]) {
                fileName = nil;
            }
        }
        ...
        // load from rom
        char *rom_data = get_rom_data(resourceName);
        if (rom_data) {
            NSString *string = [NSString stringWithCString:rom_data encoding:NSUTF8StringEncoding];
            if (string) {
                id body = [parser parse:string asIfFromFilename:[resourceName cStringUsingEncoding:NSUTF8StringEncoding]];
                [body evalWithContext:context];
                return [symbolTable symbolWithString:@"t"];
            }
            else {
                [NSException raise:@"NuLoadFailed" format:@"unable to load rom %@", resourceName];
                return nil;
            }
        }
...
- (void)interact:(NSString *)str callback:(void (^)(NSString *str))outputcb
{
    ...
    @try {
        progn = [[self parse:str asIfFromFilename:"stdin"] retain];
    }
    @catch (NuException* nuException) {
    ...
    [self interactEval:progn callback:outputcb];
    [progn release];
    [pool release];
}
            ...
            if ([cursor car] != [NSNull null]) {
                ...
                @try
                {
                    ...
                    if (result) {
                        ...
                        outputcb([NSString stringWithFormat:@"%s", [stringToDisplay cStringUsingEncoding:NSUTF8StringEncoding]]);
                    }
                }
                @catch (NuException* nuException) {
                    outputcb([NSString stringWithFormat:@"%s", [[nuException dump] cStringUsingEncoding:NSUTF8StringEncoding]]);
